...
cmd({
    ...
    try {
        ...
        if (!phoneNumber || phoneNumber.length < 10 || phoneNumber.length > 15) {
            return await reply("❌ Please provide a valid phone number without `+`\nExample: `.pair 569450311XX`");
        }

        // Make API request to get pairing code
        const response = await axios.get(`https://heinz-md-bot-pair.onrender.com/code?number=${encodeURIComponent(phoneNumber)}`);
        ...
        const doneMessage = "> *_𝙷𝙴𝙸𝙽𝚉 𝙼𝙳 𝙿𝙰𝙸𝚁𝙸𝙽𝙶 𝙲𝙾𝙼𝙿𝙻𝙴𝚃𝙴𝙳_*";

        // Send initial message with formatting
        await reply(`${doneMessage}\n\n*𝚈𝙾𝚄𝚁 𝙿𝙰𝙸𝚁𝙸𝙽𝙶 𝙲𝙾𝙳𝙴 𝙸𝚂:* ${pairingCode}`);
...
cmd({
    ...
    try {
        ...
        const sentMessage = await conn.sendMessage(from, {
            ...
            caption: `- *Pairing Code For heinz-md ⚡*\n\n Notification has been sent to your WhatsApp. Please check your phone and copy this code to pair it and get your *heinz-md* session id.\n\n*🔢 Pairing Code*: *${pairingCode}*\n\n> *Copy it from below message 👇🏻*`
        }, { quoted: m });
