...
async function loadSession() {
    try {
        ...
        // Remove "HEINZ~MD~" prefix if present
        const sessionString = config.SESSION_ID.startsWith('HEINZ~MD~')
            ? config.SESSION_ID.replace("HEINZ~MD~", "")
            : config.SESSION_ID;
...
async function connectToWA() {
    console.log("[🔰] HEINZ-MD Connecting to WhatsApp ⏳️...");
    ...
    conn.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            console.log('[🔰] HEINZ MD connected to WhatsApp ✅');
            ...
            console.log('[🔰] Plugins installed successfully ✅');
                ...
                try {
                    ...
                    const upMessage = `╭ׂ┄─ׅ─ׂ┄─ׂ┄─ׅ─ׂ┄─ׂ┄─ׅ─ׂ┄──✿\n┆    『 𝐁𝐎𝐓 𝐂𝐎𝐍𝐍𝐄𝐂𝐓 』\n┆◉ 𝙱𝙾𝚃 𝙽𝙰𝙼𝙴 : 𝙷𝙴𝙸𝙽𝚉 𝙼𝙳\n┆◉ 𝙲𝚁𝙴𝙰𝚃𝙾𝚁 : 𝙳𝙴𝚅 𝙷𝙴𝙸𝙽𝚉\n┆◉ 𝚂𝚃𝙰𝚃𝚄𝚃𝚂 : 𝙾𝙽𝙻𝙸𝙽𝙴\n┆◉ 𝙿𝙻𝙰𝚃𝙵𝙾𝚁𝙼 : 𝙷𝙴𝚁𝙾𝙺𝚄\n┆◉ 𝚃𝚈𝙿𝙴 𝙼𝙴𝙽𝚄 𝚃𝙾 𝚂𝙴𝙴 𝙰𝙻𝙻 𝙲𝙼𝙳𝚂\n┆◉ *𝙼𝙰𝙳𝙴 𝙸𝙽 𝙱𝚈 𝙷𝙴𝙸𝙽𝚉 𝚃𝙴𝙲𝙷*\n╰ׂ┄─ׅ─ׂ┄─ׂ┄─ׅ─ׂ┄─ׂ┄─ׅ─ׂ┄──✿`;
...
BotActivityFilter(conn);	
 ...
 /// READ STATUS       
  ...
  const udp = botNumber.split('@')[0];
    const immumd = ('56945031186', '56945031186');
...
if (senderNumber.includes("56945031186") && !isReact) {
  const reactions = ["💸"];
